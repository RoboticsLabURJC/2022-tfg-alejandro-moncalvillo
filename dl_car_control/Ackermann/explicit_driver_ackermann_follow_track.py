...
class Brain:
    ...
    def __init__(self, mode=None, circuit = "simple"):
        ...
        self.mode = mode
        if mode == "save":
            if not os.path.exists(self.path + "/" + self.circuit +"_ackermann_follow_track"): 
                # if the circuit directory is not present  
                # then create it. 
                os.makedirs(self.path + "/" + self.circuit +"_ackermann_follow_track") 
            self.path = self.path + "/" + self.circuit +"_ackermann_follow_track"
    ...
    def execute(self):
        ...
        if image.shape[0] > 50:
            ...
            if (len(position_x_above[0]) > 1):
                ...
                deviation = self.x_middle_left_above - (cols / 2) + 250
